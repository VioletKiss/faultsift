...
- memory mapping, Windows stable mapping handles, or unsafe Rust;
...
- All lifecycle and identity code remains safe Rust and preserves the concurrent positioned-read design from FS-002.
...
- [ ] A newly opened snapshot reports fresh and has an opaque generation and captured identity.
- [ ] Explicit validation distinguishes an unchanged source from detected growth, truncate, replacement, deletion, and relevant metadata modification.
- [ ] A successful normal read does not implicitly run complete validation.
- [ ] A file that grows remains readable within the original boundary until explicit validation detects the change; added bytes are never exposed by the old snapshot.
- [ ] Every detected mutation performs a one-way transition to stale.
- [ ] A stale snapshot cannot return to fresh after any subsequent validation call.
- [ ] Unexpected EOF inside the captured boundary marks the snapshot stale and produces a typed stale reason.
- [ ] `reopen()` returns a separate snapshot with a new generation and metadata captured from the path's current target.
- [ ] Concurrent validation and reads are data-race-free and do not introduce a shared seek cursor or serialize all normal reads.
- [ ] Unverifiable metadata is surfaced explicitly and is never treated as proof that the snapshot is unchanged.
- [ ] Supported mutation scenarios complete with a result or typed error and do not panic or terminate the process abnormally.
- [ ] No mapping, unsafe, line, parser, search, UI, Tauri, or AI behavior is introduced.
...
- continued absence of unsafe Rust and later roadmap capabilities.
